...
class JackalJacoMobimanDRL(jackal_jaco_env.JackalJacoEnv):
    ...
    def __init__(self, robot_id=0, data_folder_path=""):
        ...
        rospy.Subscriber(self.config.extcoldistance_arm_msg_name, collision_info, self.callback_extcoldistance_arm) # type: ignore
    ...
    def _is_done(self, observations):
        ...
        #print("[jackal_jaco_mobiman_drl::JackalJacoMobimanDRL::_is_done] termination_reason: " + self.termination_reason) # type: ignore
    ...
    def _compute_reward(self, observations, done):
        ...
        if self._episode_done and (not self._reached_goal):
            ...
            print("[jackal_jaco_mobiman_drl::JackalJacoMobimanDRL::_compute_reward] weighted_reward_step_target2goal: " + str(weighted_reward_step_target2goal))
        ...
        if self._episode_done and self.episode_num > 0:
            #print("[jackal_jaco_mobiman_drl::JackalJacoMobimanDRL::_is_done] episode_num: " + str(self.episode_num))
            ...
            print("[jackal_jaco_mobiman_drl::JackalJacoMobimanDRL::_is_done] AND THE NEW total_mean_episode_reward!!!")
            self.total_mean_episode_reward = (self.total_mean_episode_reward * (self.episode_num - 1) + self.episode_reward) / self.episode_num

            #print("[jackal_jaco_mobiman_drl::JackalJacoMobimanDRL::_is_done] DEBUG INF")
            #while 1:
            #    continue
        ...
        print("[jackal_jaco_mobiman_drl::JackalJacoMobimanDRL::_compute_reward] ablation_mode: {}".format(self.config.ablation_mode))
        print("[jackal_jaco_mobiman_drl::JackalJacoMobimanDRL::_compute_reward] mpc_action_result: {}".format(self.mpc_action_result))
        print("[jackal_jaco_mobiman_drl::JackalJacoMobimanDRL::_compute_reward] termination_reason: {}".format(self.termination_reason))
        ...
        print("[jackal_jaco_mobiman_drl::JackalJacoMobimanDRL::_compute_reward] total_time_horizon: {}".format(self.total_time_horizon))
    ...
    def callback_extcoldistance_base(self, msg):
        #print("[jackal_jaco_mobiman_drl::JackalJacoMobimanDRL::callback_extcoldistance_base] INCOMING")
    ...
    def callback_extcoldistance_arm(self, msg):
        #print("[jackal_jaco_mobiman_drl::JackalJacoMobimanDRL::callback_extcoldistance_arm] INCOMING")
    ...
    def check_goal(self):
        ...
        quatdistance2goal = self.get_arm_quatdistance2goal()

        goal_distance_ori_threshold = self.config.goal_distance_ori_threshold_quat
        if self.model_mode == 0:
            goal_distance_ori_threshold = self.config.goal_distance_ori_threshold_yaw
        #print("[jackal_jaco_mobiman_drl::JackalJacoMobimanDRL::check_goal] distance2goal_ee: " + str(distance2goal_ee))        
        #print("[jackal_jaco_mobiman_drl::JackalJacoMobimanDRL::check_goal] goal_distance_pos_threshold: " + str(self.config.goal_distance_pos_threshold))        
        #print("[jackal_jaco_mobiman_drl::JackalJacoMobimanDRL::check_goal] goal_distance_ori_threshold: " + str(self.config.goal_distance_ori_threshold))        
        if (distance2goal_ee < self.config.goal_distance_pos_threshold and quatdistance2goal < goal_distance_ori_threshold): # type: ignore
            self._episode_done = True
            self._reached_goal = True
            print("[jackal_jaco_mobiman_drl::JackalJacoMobimanDRL::check_goal] REACHED TO THE GOAL!")
    ...
    def initialize_robot_pose(self, init_flag=True):
        ...
        if self.config.world_name == "conveyor":
            ...
            p1 = {"x":self.world_range_x_min, "y":self.world_range_y_min, "z":self.world_range_z_min}
            p2 = {"x":self.world_range_x_max, "y":self.world_range_y_max, "z":self.world_range_z_max}
    ...
    def get_base_yawdistance2target(self):
        ...
        '''
        if self.model_mode == 0 and distance2goal > 0.5:
            print("[jackal_jaco_mobiman_drl::JackalJacoMobimanDRL::get_arm_quatdistance2goal] target_data yaw: " + str(self.goal_data["yaw"]))
            print("[jackal_jaco_mobiman_drl::JackalJacoMobimanDRL::get_arm_quatdistance2goal] robot_data yaw: " + str(self.arm_data["yaw"]))

        if self.model_mode == 0 and distance2goal > 3*math.pi/2:
            print("[jackal_jaco_mobiman_drl::JackalJacoMobimanDRL::get_base_yawdistance2target] model_mode: " + str(self.model_mode))
            print("[jackal_jaco_mobiman_drl::JackalJacoMobimanDRL::get_base_yawdistance2target] target_data yaw: " + str(self.target_data["yaw"]))
            print("[jackal_jaco_mobiman_drl::JackalJacoMobimanDRL::get_base_yawdistance2target] robot_data yaw: " + str(self.robot_data["yaw"]))
            print("[jackal_jaco_mobiman_drl::JackalJacoMobimanDRL::get_base_yawdistance2target] distance2goal: " + str(distance2goal))

            quatdistance2goal = self.get_quaternion_distance(self.target_data, self.robot_data)
            print("[jackal_jaco_mobiman_drl::JackalJacoMobimanDRL::get_base_yawdistance2target] quatdistance2goal: " + str(quatdistance2goal))

            print("[jackal_jaco_mobiman_drl::JackalJacoMobimanDRL::get_base_yawdistance2target] DEBUG INF")
            while 1:
                continue
        '''
    ...
    def reward_step_target2goal_diff_func(self, curr_target2goal, prev_target2goal):
        ...
        reward_step_target2goal = self.config.reward_step_target2goal * self.gaussian_function(diff_target2goal-self.config.reward_step_target2goal_mu_regular, self.config.reward_step_target2goal_sigma_regular)
    ...
    def reward_step_target2goal_curr_func(self, curr_target2goal):
        reward_step_target2goal = self.config.reward_step_target2goal * self.gaussian_function(curr_target2goal-self.config.reward_step_target2goal_mu_last_step, self.config.reward_step_target2goal_sigma_last_step)
        return reward_step_target2goal
    
    def reward_step_target2goal_func(self, curr_target2goal, prev_target2goal):
        distance2goal = self.get_base_distance2goal_2D()
        if distance2goal < self.config.last_step_distance_threshold: # type: ignore
            print("[jackal_jaco_mobiman_drl::JackalJacoMobimanDRL::reward_step_target2goal_func] WITHIN LAST STEP DISTANCE!")
    ...
    def check_collision(self):
        ...
        for dist in selfcoldistance:
            #print("[jackal_jaco_mobiman_drl::JackalJacoMobimanDRL::check_collision] selfcoldistance dist: " + str(dist))
        ...
        for dist in extcoldistance_base:
            #print("[jackal_jaco_mobiman_drl::JackalJacoMobimanDRL::check_collision] extcoldistance_base dist: " + str(dist))
            if dist < self.config.ext_collision_range_base_min:
                print("[jackal_jaco_mobiman_drl::JackalJacoMobimanDRL::check_collision] EXT BASE COLLISION")
                self._episode_done = True
                self.termination_reason = 1
                return True
            
        for dist in extcoldistance_arm:
            #print("[jackal_jaco_mobiman_drl::JackalJacoMobimanDRL::check_collision] extcoldistance_arm dist: " + str(dist))
            if dist < self.config.ext_collision_range_arm_min:
                print("[jackal_jaco_mobiman_drl::JackalJacoMobimanDRL::check_collision] EXT ARM COLLISION")
                self._episode_done = True
                self.termination_reason = 1
                return True

        for por in pointsonrobot:
            if por.z < self.config.ext_collision_range_base_min:
    ...
    def get_obs_selfcoldistance(self):
        ...
        obs_selfcoldistance = np.full((1, self.config.n_selfcoldistance), self.config.self_collision_range_max).reshape(self.config.fc_obs_shape) # type: ignore
    ...
    def get_obs_extcoldistance_base(self):
        ...
        obs_extcoldistance_base = np.array([[extcoldistance_base_msg.p1[0].x, 
                                             ...
                                             extcoldistance_base_msg.p1[0].z]]).reshape(self.config.fc_obs_shape)
    ...
    def get_obs_extcoldistance_arm(self):
        ...
        obs_extcoldistance_arm = np.array([[extcoldistance_arm_msg.p1[0].x, 
                                            ...
                                            extcoldistance_arm_msg.p1[0].z]]).reshape(self.config.fc_obs_shape)
    ...
    def get_obs_armstate(self):
        ...
        obs_armstate = np.full((1, self.config.n_armstate), 0).reshape(self.config.fc_obs_shape) # type: ignore
    ...
    def init_observation_action_space(self):
        ...
        if self.config.observation_space_type == "mobiman_FC":
            ...
            print("[jackal_jaco_mobiman_drl::JackalJacoMobimanDRL::init_observation_action_space] obs_selfcoldistance_min shape: " + str(obs_selfcoldistance_min.shape))
            print("[jackal_jaco_mobiman_drl::JackalJacoMobimanDRL::init_observation_action_space] obs_collision_base_min shape: " + str(obs_collision_base_min.shape))
            print("[jackal_jaco_mobiman_drl::JackalJacoMobimanDRL::init_observation_action_space] obs_collision_arm_min shape: " + str(obs_collision_arm_min.shape))
            ...
            print("[jackal_jaco_mobiman_drl::JackalJacoMobimanDRL::init_observation_action_space] obs_armstate_min shape: " + str(obs_armstate_min.shape))
            ...
            print("[jackal_jaco_mobiman_drl::JackalJacoMobimanDRL::init_observation_action_space] NEEDS REVIEW: DEBUG INF")
            while 1:
                continue
    ...
    def reinit_observation(self):
        ...
        if self.config.observation_space_type == "mobiman_FC":
            ...
            self.obs_data = {   "selfcoldistance": np.vstack([obs_selfcoldistance] * (self.config.n_obs_stack * self.config.n_skip_obs_stack)), # type: ignore
                                "extcoldistance_base": np.vstack([obs_extcoldistance_base] * (self.config.n_obs_stack * self.config.n_skip_obs_stack)), # type: ignore
                                "extcoldistance_arm": np.vstack([obs_extcoldistance_arm] * (self.config.n_obs_stack * self.config.n_skip_obs_stack)), # type: ignore
            ...
            print("[jackal_jaco_mobiman_drl::JackalJacoMobimanDRL::reinit_observation] NEEDS REVIEW: DEBUG INF")
            while 1:
                continue
            ...
            self.obs_data = {   "occgrid_image": np.vstack([obs_occgrid_image] * (self.config.n_obs_stack * self.config.n_skip_obs_stack)), # type: ignore
                                "selfcoldistance": np.vstack([obs_selfcoldistance] * (self.config.n_obs_stack * self.config.n_skip_obs_stack)), # type: ignore
                                "extcoldistance_base": np.vstack([obs_extcoldistance_base] * (self.config.n_obs_stack * self.config.n_skip_obs_stack)), # type: ignore
                                "extcoldistance_arm": np.vstack([obs_extcoldistance_arm] * (self.config.n_obs_stack * self.config.n_skip_obs_stack)), # type: ignore
                                "goal": np.vstack([obs_goal] * (self.config.n_obs_stack * self.config.n_skip_obs_stack))} # type: ignore
            ...
            print("[jackal_jaco_mobiman_drl::JackalJacoMobimanDRL::reinit_observation] obs_data selfcoldistance shape: " + str(self.obs_data["selfcoldistance"].shape))
            print("[jackal_jaco_mobiman_drl::JackalJacoMobimanDRL::reinit_observation] obs_data extcoldistance_base shape: " + str(self.obs_data["extcoldistance_base"].shape))
            print("[jackal_jaco_mobiman_drl::JackalJacoMobimanDRL::reinit_observation] obs_data extcoldistance_arm shape: " + str(self.obs_data["extcoldistance_arm"].shape))
            ...
            print("[jackal_jaco_mobiman_drl::JackalJacoMobimanDRL::reinit_observation] obs_space_coldistance_goal shape: " + str(obs_space_coldistance_goal.shape))
            print("---------------------")

            self.obs = {"occgrid_image": obs_space_occgrid_image,
                        "coldistancedist_goal": obs_space_coldistance_goal}
    ...
    def update_observation(self):
        ...
        if self.config.observation_space_type == "mobiman_FC":
            ...
            #print("[jackal_jaco_mobiman_drl::JackalJacoMobimanDRL::update_observation] obs_data selfcoldistance shape: " + str(self.obs_data["selfcoldistance"].shape))
            ...
            print("[jackal_jaco_mobiman_drl::JackalJacoMobimanDRL::update_observation] NEEDS REVIEW: DEBUG INF")
            while 1:
                continue
            ...
            #print("[jackal_jaco_mobiman_drl::JackalJacoMobimanDRL::update_observation] obs_space_coldistance_goal shape: " + str(obs_space_coldistance_goal.shape))
            #print("****************")
    ...
    def client_set_action_drl(self, action, last_step_flag=False):
        ...
        try:
            if self.config.action_type == 0:
                ...
                success = srv_set_discrete_action_drl(action, self.config.action_time_horizon).success
            else:
                ...
                success = srv_set_continuous_action_drl(action, self.config.action_time_horizon, last_step_flag, self.config.last_step_distance_threshold).success
